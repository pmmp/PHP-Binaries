...
if [ "$IS_CROSSCOMPILE" == "yes" ]; then
	...
	if [[ "$COMPILE_TARGET" == "win" ]] || [[ "$COMPILE_TARGET" == "win64" ]]; then
		...
		export ac_cv_have_shm_mmap_anon=yes
		export ac_cv_have_shm_mmap_posix=yes
...
if [[ "$HAVE_XDEBUG" == "yes" ]]; then
	...
	./configure --with-php-config="$INSTALL_DIR/bin/php-config" $CONFIGURE_FLAGS >> "$DIR/install.log" 2>&1
	echo -n " compiling..."
